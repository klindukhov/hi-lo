...
export default function App() {
  const [history, setHistory] = useState([
    new GameState("", new PlayingCard("", "", "-"), "", 0, 30),
  ]);
  ...
  useEffect(() => {
    ...
    if (state[state.length - 1].roundsLeft < 1) {
      endGame();
      return;
    }
    setIsContinue(true);
  ...
  const handleBet = (bet: string) => {
    ...
    getNextCard(currentState.deckId)
      .then((res) => {
        if (!res) {
          setIsFetchError(true);
          return;
        ...
        if (
          bet === "0" &&
          currentState.currentCard.getNumericValue() < card.getNumericValue()
        )
          p++;
        if (
          bet === "1" &&
  ...
  return (
    <React.Fragment>
      <Dialog open={isFetchError}>
        ...
        <DialogContent>
          It seems that we cant get a deck for you.
          <br /> Check your internet connection, or try again later.
        </DialogContent>
        <DialogActions>
      ...
      <MainPage>
        <Column>
          ...
          {history &&
            (history as []).map((e: any, i: number) => (
              <HistoryPanel
                key={e.currentCard.code}
                entry={[
                  i + 1,
                  e.currentCard.code,
                  e.points / 10,
                  e.bet === "0" ? "High" : e.bet === "1" ? "Low" : "-",
                ]}
              />
            ))}
        </Column>
        <CenterColumn>
          <Card
            currentCard={history[history.length - 1].currentCard}
            isGameOn={isGameOn}
          />
          <BetPanel
            handleBet={handleBet}
        ...
        <Column>
          <ScorePanel
            ...
            roundsLeft={history[history.length - 1].roundsLeft}
          />
        </Column>
      </MainPage>
    </React.Fragment>
